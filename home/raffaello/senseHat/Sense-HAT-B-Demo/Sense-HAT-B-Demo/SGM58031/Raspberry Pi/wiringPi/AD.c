#include <wiringPi.h>
#include <wiringPiI2C.h>
#include <linux/i2c-dev.h>
#include <sys/ioctl.h>
#include <sys/types.h>
#include <stdio.h>
#include <math.h>
#include"AD.h"
int Config_Set;
int fd; 
int I2C_readU16(int reg)
{   int val;
    unsigned char Val_L,Val_H;
    val=wiringPiI2CReadReg16(fd,reg);                    //High and low bytes are the opposite       
    Val_H=val&0xff;
    Val_L=val>>8;
    val=(Val_H<<8)|Val_L;                               //Correct byte order
    return val;
}
void I2C_writeWord(int reg, int val)
{   unsigned char Val_L,Val_H;
    Val_H=val&0xff;
    Val_L=val>>8;
    val=(Val_H<<8)|Val_L;                               ////Correct byte order
	wiringPiI2CWriteReg16(fd,reg,val);
}
unsigned int SGM58031_INIT()
{   unsigned int state;
    state=I2C_readU16(SGM_POINTER_CONFIG) & 0x8000  ;
    return state;
}
unsigned int SGM58031_SINGLE_READ(unsigned int channel)           //Read single channel data
{   unsigned int data;
    Config_Set = SGM_CONFIG_MODE_NOCONTINUOUS        |   //mode：Single-shot mode or power-down state    (default)
                 SGM_CONFIG_PGA_4096                 |   //Gain= +/- 4.096V                              (default)
                 SGM_CONFIG_COMP_QUE_NON             |   //Disable comparator                            (default)
                 SGM_CONFIG_COMP_NONLAT              |   //Nonlatching comparator                        (default)
                 SGM_CONFIG_COMP_POL_LOW             |   //Comparator polarity：Active low               (default)
                 SGM_CONFIG_COMP_MODE_TRADITIONAL    |   //Traditional comparator                        (default)
                 SGM_CONFIG_DR_RATE_480             ;    //Data rate=480Hz                             (default)
    switch (channel)
    {
        case (0):
            Config_Set |= SGM_CONFIG_MUX_SINGLE_0;
            break;
        case (1):
            Config_Set |= SGM_CONFIG_MUX_SINGLE_1;
            break;
        case (2):
            Config_Set |= SGM_CONFIG_MUX_SINGLE_2;
            break;
        case (3):
            Config_Set |= SGM_CONFIG_MUX_SINGLE_3;
            break;
    }
    Config_Set |=SGM_CONFIG_OS_SINGLE_CONVERT;
    I2C_writeWord(SGM_POINTER_CONFIG,Config_Set);
    delay(20);
    data=I2C_readU16(SGM_POINTER_CONVERT);
    return data;
}
int main()
{
	int  AIN0_DATA,AIN1_DATA,AIN2_DATA,AIN3_DATA;
    wiringPiSetup();
    fd=wiringPiI2CSetup(SGM_I2C_ADDRESS);
	printf("\nSGM58031 Test Program ...\n");
    if(SGM58031_INIT()!=0x8000)
	{	printf("\nSGM58031 Error\n");
		return 0;
	}
	delay(10);
	printf("\nSGM58031 OK\n");
    while(1)
    {
        AIN0_DATA=SGM58031_SINGLE_READ(0);
        AIN1_DATA=SGM58031_SINGLE_READ(1);
        AIN2_DATA=SGM58031_SINGLE_READ(2);
        AIN3_DATA=SGM58031_SINGLE_READ(3);
        printf("\nAIN0 = %d(%0.2fmv) ,AIN1 = %d(%0.2fmv) ,AIN2 = %d(%0.2fmv) AIN3 = %d(%0.2fmv)\n\r", AIN0_DATA,(float)AIN0_DATA*0.125, AIN1_DATA,(float)AIN1_DATA*0.125,AIN2_DATA,(float)AIN2_DATA*0.125,AIN3_DATA,(float)AIN3_DATA*0.125);
        delay(500);
    }
    return 1;
}