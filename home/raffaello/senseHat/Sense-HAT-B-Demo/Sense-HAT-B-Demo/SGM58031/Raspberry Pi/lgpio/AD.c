...
unsigned int SG58031_INIT()
{   unsigned int state;
    state=I2C_readU16(SGM_POINTER_CONFIG) & 0x8000  ;
    return state;
}
unsigned int SG58031_SINGLE_READ(unsigned int channel)           //Read single channel data
{   unsigned int data;
    ...
    I2C_writeWord(SGM_POINTER_CONFIG,Config_Set);
    lguSleep(0.02);
    data=I2C_readU16(SGM_POINTER_CONVERT);
    return data;
}
int main()
{
	int  AIN0_DATA,AIN1_DATA,AIN2_DATA,AIN3_DATA;
    fd=lgI2cOpen(1,SGM_I2C_ADDRESS,0);
	printf("\nSG58031 Test Program ...\n");
    if(SG58031_INIT()!=0x8000)
	{	printf("\nSG58031 Error\n");
		return 0;
	}
	lguSleep(0.01);
    ...
    while(1)
    {
        AIN0_DATA=SG58031_SINGLE_READ(0);
        AIN1_DATA=SG58031_SINGLE_READ(1);
        AIN2_DATA=SG58031_SINGLE_READ(2);
        AIN3_DATA=SG58031_SINGLE_READ(3);
