...
IMU_ST_SENSOR_DATA gstMagnOffset = {-98, 71, -148};
#ifdef __cplusplus
extern "C" {
#endif
...
{
  return lgI2cReadByteData(fd,reg);
}

void I2C_WriteOneByte(char reg, char val)
{
	lgI2cWriteByteData(fd, reg, val);
}
/******************************************************************************
 * IMU module                                                                 *
 ******************************************************************************/
...
void imuInit(IMU_EN_SENSOR_TYPE *penMotionSensorType)
{
  ...
  fd = lgI2cOpen(1,I2C_ADD_ICM20948,0);
  bRet = icm20948Check();
  if( true == bRet)
  {
...
float invSqrt(float x) 
{
  ...
  float y = 0;
  y = x;
  long i = 0;                //get bits for floating value
  i = *(long*)&y;
...
void icm20948MagRead(short int* ps16X, short int* ps16Y, short int* ps16Z)
{
    ...
    *ps16X =  s32OutBuf[0] - gstMagnOffset.s16X;
    *ps16Y = -s32OutBuf[1] - gstMagnOffset.s16Y;
    *ps16Z = -s32OutBuf[2] - gstMagnOffset.s16Z;
    return;
}
...
bool icm20948MagnOffset(void)
{
  ...
  while(s8CaliCounter != 3)
  {
    ...
    {
      case 0:
        printf("Please place it horizontally 10dof-D\n");
        printf("If you have completed the action, press Enter on your keyboard\n");
        getchar();
        lguSleep(0.1);
        icm20948MagRead(&s16MagnBuff[0], &s16MagnBuff[1], &s16MagnBuff[2]);
        lguSleep(0.1);
        break;
      case 1:
        ...
        getchar();
        lguSleep(0.1);        
        icm20948MagRead(&s16MagnBuff[3], &s16MagnBuff[4], &s16MagnBuff[5]);
        lguSleep(0.1);
        break;
      case 2:
        ...
        getchar();
        lguSleep(0.1);        
        icm20948MagRead(&s16MagnBuff[6], &s16MagnBuff[7], &s16MagnBuff[8]);
        lguSleep(0.1);    
        break;
      default:
  ...
  /* */
	if(s8CaliCounter == 3)
	{ 
			gstMagnOffset.s16X=(s16MagnBuff[0]+s16MagnBuff[3])/2;
			gstMagnOffset.s16Y=(s16MagnBuff[1]+s16MagnBuff[4])/2;
			gstMagnOffset.s16Z=(s16MagnBuff[5]+s16MagnBuff[8])/2;
	}
  /* */
  printf("Magnetic calibration data : x:%d, y:%d, z:%d\n",gstMagnOffset.s16X, gstMagnOffset.s16Y, gstMagnOffset.s16Z);
  return bRet;
}
